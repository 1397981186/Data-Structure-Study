...
拓展：01背包问题用贪心解决不能避免一些特殊的情况（可以自行查阅），所以那个算法教材才说01背包要用dp。
...
#### 贪心算法很多时候都不能得到全局最优，那么贪心算法的意义是什么？

**回答1：**

贪心算法的目的并不是寻找全局最优解。贪心算法的本质是基于heuristics。说到heuristic就一定会有比较主观并且比较常见的一些现象。就比如两点之间直线肯定是最短的，但是在重庆开车可能这一常识不太适用等。

所谓寻找全局最优解，第一种方法是通过论证某一种方法，在某些条件下一定是最优的，就比如绝大多数的搜索问题或者图论问题，这些实际上几十年前就已经在论文里面论证过其最优性，不论是从复杂度还是从逻辑本身。第二种方法是穷举，在计算机进行无数次模拟之后找出的[最优解](https://www.zhihu.com/search?q=最优解&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A2284312569})。理论上穷举一定可以找出全局最优，但无限的穷举在一些问题显然是不存在的。

个人认为贪心最主要的目的是简化问题。在简化复杂的条件后找出相对更优的一种方案。其实有点类似强化学习里面回馈函数的设置。实际上让一个RL模型快速拟合主要还是要靠一些基于[heuristic](https://www.zhihu.com/search?q=heuristic&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"answer"%2C"sourceId"%3A2284312569})的东西（当然大致可以理解成贪心），否则除非用对抗训练可能会好一点，不然模型很难拟合。
